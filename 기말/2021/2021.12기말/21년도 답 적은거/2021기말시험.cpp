...
//--------
{
	...
	for (int i = 0; i < num; ++i) {
		...
		{
		case 0:
			monster[i] = new TimeMonster(uiTime(dre));
			break;
		case 1:
			monster[i] = new MemoryMonster(uiMemory(dre));
			break;
		case 2:
			monster[i] = new ScreenMonster(uiScreen(dre));
			break;
		default:
			break;
		}
	...
	for (int i = 0; i < num; ++i) {
		if (dynamic_cast<MemoryMonster*>(monster[i])) {
			monster[i]->special();
		}
	}
	...
	for (int i = 0; i < num; ++i) {
		if (MemoryMonster* memoryMonster = dynamic_cast<MemoryMonster*>(monster[i])) {
			memoryMonster->show();
		}
	}
	...
	// 문제점 : [문제 6]에서 monster[0]에 이미 메모리를 지웠기 때문에 [문제 7]에서 한번 더 지우면서 댕글링이 일어날 수 있다.
	// 문제점 : 
