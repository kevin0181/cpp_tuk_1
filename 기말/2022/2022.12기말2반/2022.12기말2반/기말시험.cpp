...
void fightingKorea(Game**, int);

void fightingKorea(Game** games, int num) {
	for (int i = 0; i < num; ++i) {
		Player* player = dynamic_cast<Player*>(games[i]);
		if (player != nullptr && player->getNation() == 0) {
			player->setHp(1000);
		}
	}
}
